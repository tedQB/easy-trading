...
def longhubang(date: str = "20210430") -> pd.DataFrame:
    """
    问财-热门股票排名
    ...
    temp_df.rename(
        columns={
            "index": "序号",
            f"上榜原因[{rank_date_str}]": "上榜原因",
            f"个股热度[{rank_date_str}]": "个股热度",
            "code": "股票代码",
            f"营业部买入金额合计[{rank_date_str}]": "营业部买入金额合计",
            f"营业部交易日期[{rank_date_str}]": "营业部交易日期",
            f"营业部卖出金额合计[{rank_date_str}]": "营业部卖出金额合计",
            f"龙虎榜[{rank_date_str}]": "龙虎榜",
            f"开盘价:不复权[{rank_date_str}]":"开盘价",
            f"收盘价:不复权[{rank_date_str}]": "收盘价",
            f"最低价:不复权[{rank_date_str}]": "最低价",
            f"最高价:不复权[{rank_date_str}]": "最高价",
            "market_code": "_",
            "最新涨跌幅": "涨跌幅",
            "最新价": "现价",
            "股票代码": "_",
...
def gainian_rank(date: str = "20210430") -> pd.DataFrame:
    """
    问财-热门股票排名
    http://www.iwencai.com/unifiedwap/result?w=%E7%83%AD%E9%97%A85000%E8%82%A1%E7%A5%A8&querytype=stock&issugs&sign=1620126514335
    :param date: 查询日期
...
def stock_hot_rank_all_data(date: str = "20210430") -> pd.DataFrame:
    """
    问财-热门股票排名
    http://www.iwencai.com/unifiedwap/result?w=%E7%83%AD%E9%97%A85000%E8%82%A1%E7%A5%A8&querytype=stock&issugs&sign=1620126514335
    :param date: 查询日期
