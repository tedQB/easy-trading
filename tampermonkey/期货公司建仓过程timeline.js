...
(function () {
    ...
    $("#mainContent").before("<div class='bt3'><input type='text' style='width:60px' id='jiage' /><button id='cal'>计算</button><div class='jiantou'><span id='jia'>↑</span><span id='jian'>↓</span></div></div>")
    ...
    $("#mainContent").before("<div class='bt6'><a href='http://localhost:8080/school2/jiesebang/app3.php?future=" + $('#futures_contract').val() + "&num=true' target='_blank'>盈亏数据</a></div>")
    $("#mainContent").before("<div class='bt7'><a href='http://localhost:8080/school2/jiesebang/app4.php?future=" + $('#futures_contract').val() + "&num=false' target='_blank'>均价数据</a></div>")
    ...
    $('#clean').click(function () {
        ...
        var riqi1 = $('#riqi').val() || $('meta[name=publishid]').attr('content');
        $.post('http://localhost:8080/school2/jiesebang/getTodaylongshort.php', {
            riqi: riqi1, //rb1905
            futureName: $('#futures_contract').val()
        }, function (data) {
            $.each(data.long, function (index, obj) {
                $(".cifcoName").each(function (index, obj2) {
                    if ($(obj2).html() === obj.cifcoName) {
                        var ds = $(obj2).parent();
                        ds.removeClass().addClass('red');
                        console.log(obj.longEvePrice);
                        ds.find('.o1').html(obj.longEvePrice);
                        ds.find('.o2').html(obj.shortEvePrice);
                        ds.find('.riqi').html(obj.riqi);
                    }
                })
            });
            $.each(data.short, function (index, obj) {
                $(".cifcoName").each(function (index, obj2) {
                    if ($(obj2).html() === obj.cifcoName) {
                        var ds = $(obj2).parent();
                        ds.removeClass().addClass('green');
                        ds.find('.o1').html(obj.longEvePrice);
                        ds.find('.o2').html(obj.shortEvePrice);
                        ds.find('.riqi').html(obj.riqi);
                    }
    ...
    $('#jia').click(function () {
        var normal = parseInt($("#jiage").val()) + 1;
        $("#jiage").val(normal)
        console.log(normal);
        calculate(normal);
