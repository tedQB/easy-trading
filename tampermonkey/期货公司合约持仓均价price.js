// ==UserScript==
// @name         期货公司合约持仓均价price
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  try to take over the world!
// @author       You
// @match        http://data.eastmoney.com/futures/sh/price.html*
// @grant        none
// ==/UserScript==
   ...
   $.each($.getCache(fName),function(index,obj){
    ...
    $('.s7 div').each(function(index,obj){
        $(obj).live('click',function(){
            ...
            var dataurl = "http://datainterface.eastmoney.com/EM_DataCenter/XML.aspx?type=QHLHB&style=QHLHB&stat=4&mkt=" +$("#futures_exchange option:selected").val()+ "&code=" + $("#futures_contract option:selected").val() + "&cmd=" +  $(this).attr('value') + "&name=4";
            so.addVariable('data_file', encodeURIComponent(dataurl));
            so.write("chart_price");
            // 其他数据
            $.ajax({
                scriptCharset: "utf-8",
                //http://datainterface.eastmoney.com/EM_DataCenter/JS.aspx?type=QHCC&sty=QHSYCC&stat=4&mkt=069001007&sc=J&code=j1809&cmd=80102901&name=3&cb=callback&callback=callback&_=1532960800929
                url: "http://datainterface.eastmoney.com/EM_DataCenter/JS.aspx?type=QHCC&sty=QHSYCC&stat=4&mkt=" + $("#futures_exchange option:selected").val() + "&sc=" + $("#futures_variety option:selected").val() + "&code=" + $("#futures_contract option:selected").val() + "&cmd=" +  $(this).attr('value') + "&name=3&cb=callback",
                dataType: "jsonp",
                ...
                beforeSend: function () {
                    ...
                    if (document.all) {
                        byId('topTit').innerText = titStr + "合约持仓均价";
                    } else {
                        byId('topTit').textContent = titStr + "合约持仓均价";
                    }
                    document.title = titStr + "合约持仓均价 _ 数据中心 _ 东方财富网";
                },
                success: function (data) {
                    if (data[0].stats == undefined) {
                        // 持仓量
                        var chart2 = emChart.models.BarChart({
                            container: "chart_bsv",
                            chartdata: data[0].series2,
                            xLableSpace: 5,
                            d3enabled: false
                        });
                        // 净持仓量
                        var chart3 = emChart.models.BarChart({
                            container: "chart_net",
                            chartdata: data[0].series3,
                            xLableSpace: 5,
                            d3enabled: false
                        });
                    } else {
                        ...
                        var chart2 = emChart.models.BarChart({
                            container: "chart_bsv",
                            chartdata: emptyData.cclEmptyData,
                            xLableSpace: 5,
                            d3enabled: false
                        });
                        // 净持仓量
                        var chart3 = emChart.models.BarChart({
                            container: "chart_net",
                            chartdata: emptyData.jcclEmptyData,
                            xLableSpace: 3,
                            d3enabled: false
                        });
                    }
