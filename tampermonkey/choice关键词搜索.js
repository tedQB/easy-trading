...
(function() {
    ...
    // Your code here...
      ...
      setTimeout(function(){
         ...
         $("#json").empty().append("<h2 style='margin-left:30px'>包含"+pickChannelName+"</h2>");

         $("#json").append("<div id='news'></div>");
         $.each(pickDateArray,function(index,obj){
             if(index%2 ==0){
                $("#news").append("<p style='overflow:hidden; background-color:#EFEFEF;height:30px; line-height:30px; margin:0px 0px 0px 0px; padding-left:30px;'><span style='float:left;width:50px; text-align:left'><span style='float:right; margin-right:10px'>"+pickDateArray[index]+"</span></span> <a href="+pickUrlArray[index]+" target='_blank'>"+pickTitleArray[index]+"</a></p>");
            }else{
                $("#news").append("<p style='overflow:hidden; height:30px; line-height:30px; margin:0px 0px 0px 0px; padding-left:30px;'><span style='float:left;width:50px; text-align:left'><span style='float:right; margin-right:10px'>"+pickDateArray[index]+"</span></span> <a href="+pickUrlArray[index]+" target='_blank'>"+pickTitleArray[index]+"</a></p>");
            }
         });
         ...
         $("#json").before("<div style='margin-top:20px;margin-left:30px'><input type='text' style='width:280px;height:25px' id='searchText'/ > <input type='button' value='提交' id='subText' /></div>");
